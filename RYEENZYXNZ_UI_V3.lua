...
local function Spring(obj, props, duration)
    local info = TweenInfo.new(duration or 0.4, Enum.EasingStyle.Spring, Enum.EasingDirection.Out)
    local t = TweenService:Create(obj, info, props)
    t:Play()
    return t
end
...
local function ConfigDecode(value, expectedType)
    if type(value) == "table" then
        ...
        elseif value.__type == "EnumItem" then
            if expectedType then
                local ok, result = pcall(function()
                    return expectedType[value.Name]
                end)
...
function RYEENZYXNZ_UI:CreateWindow(opts)
    ...
    local toggleConn = UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == toggleKey then
            Window._open = not Window._open
            if Window._open then
