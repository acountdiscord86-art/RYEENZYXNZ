...
local function buildTagFrame(player)
    ...
    root.Name              = "RYNZYEXZTag_" .. player.UserId
    ...
    -- "RYNZYEXZ" badge (bottom right, small pill)
...
local function applyOutline(player)
    ...
    local existing = char:FindFirstChild("RYNZYEXZOutline")
    if existing then existing:Destroy() end

    local hl = Instance.new("Highlight")
    hl.Name             = "RYNZYEXZOutline"
...
local function addTag(player)
    ...
    local conn = RunService.RenderStepped:Connect(function(dt)
        ...
        local outline = char:FindFirstChild("RYNZYEXZOutline")
...
local function tagRegister()
    ...
    if sok and type(data) == "table" and data.kick == true then
        pcall(function() lp:Kick("[RYNZYEXZ] Disconnected by admin") end)
    end
end
...
function Library:SetTheme(theme)
    ...
    if type(theme) == "string" then
        themeName = theme
        theme = THEMES[theme]
        if not theme then warn(("[RYNZYEXZ] unknown theme %q"):format(themeName)); return false end
    elseif type(theme) ~= "table" then
        warn("[RYNZYEXZ] SetTheme expects a built-in theme name or theme table"); return false
    end
    for key in pairs(C) do
        local value = theme[key]
        if value ~= nil and typeof(value) ~= "Color3" then
            warn(("[RYNZYEXZ] theme key %s must be a Color3"):format(key)); return false
        end
    end
    for key in pairs(C) do
        local value = theme[key]
...
function Library:SaveConfig(name)
    if not hasFileApi() then
        warn("[RYNZYEXZ] SaveConfig requires an executor file API (writefile)")
        return false
    end
    ensureConfigFolder()
    ...
    if not ok then warn("[RYNZYEXZ] SaveConfig failed to encode config"); return false end
    local wrote = pcall(writefile, configPath(name), encoded)
    if not wrote then warn("[RYNZYEXZ] SaveConfig failed to write file"); return false end
    return true
end
...
function Library:LoadConfig(name)
    if not hasFileApi() then
        warn("[RYNZYEXZ] LoadConfig requires an executor file API (readfile)")
        return false
    end
    ...
    if not decoded then warn("[RYNZYEXZ] LoadConfig failed to decode config"); return false end
...
function Library:Notify(opts)
    ...
    warn("[RYNZYEXZ] create a window before calling Library:Notify")
    return nil
end
...
local function buildMusicPlayer(cfg)
    ...
    local MUSIC_FOLDER   = tostring(opts.MusicFolder or "RYNZYEXZMusic")
    ...
    musicSound.Name   = "RYNZYEXZMusicPlayer"
...
function Library:CreateWindow(opts)
    ...
    local guiName        = opts.GuiName or "RYNZYEXZ"
    ...
    local loadingText         = tostring(opts.LoadingText or opts.Name or "RYNZYEXZ")
    local loadingSub          = tostring(opts.LoadingSubtitle or "HUB")
    local loadingFooter       = tostring(opts.LoadingFooter or "RYNZYEXZ")
    ...
    make("TextLabel",{Text=opts.Name or "RYNZYEXZ",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.White,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,16),Size=UDim2.new(1,-72,0,17),Parent=brand})
    make("TextLabel",{Text=opts.BrandSubtitle or ("RYNZYEXZ v"..Library.Version),Font=Enum.Font.GothamMedium,TextSize=9,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,35),Size=UDim2.new(1,-72,0,13),Parent=brand})
    ...
    circle(statusDot)
    make("TextLabel",{Text=opts.StatusText or "RYNZYEXZ is ready",Font=Enum.Font.GothamMedium,TextSize=10,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,28,1,-27),Size=UDim2.new(1,-40,0,16),Parent=sidebar})
    ...
    if isMobile then
        local fab = make("TextButton", {
            Name = "RYNZYEXZMobileToggle", Text = "", AutoButtonColor = false,
